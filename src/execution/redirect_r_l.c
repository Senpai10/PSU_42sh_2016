...
** Last update Sun May 21 23:33:40 2017 Senpai
*/

#include "execution.h"

int	reset_io_r_l(int fd_r, int fd_l, int newfd_r, int newfd_l)
{
  if ((close(fd_r)) == -1)
    return (EXEC_ERROR);
  if ((close(fd_l)) == -1)
...
int	change_io_r_l(int fd_r, int fd_l, char **args, char **envp)
{
  int	signal;
...
int     redirect_r_l(char **args, t_command **commands, char **envp)
{
  ...
  if ((fd_l = open(redir_l, O_RDONLY)) == -1)
    ...
    if ((fd_r = open(redir_r, O_CREAT | O_WRONLY, 0666)) == -1)
      return (EXEC_ERROR);
  if (((signal = change_io_r_l(fd_r, fd_l, args, envp)) == EXEC_ERROR) ||
      (reset_io_r_l(fd_r, fd_l, newfd_r, newfd_l)) == EXEC_ERROR)
    return (EXEC_ERROR);
