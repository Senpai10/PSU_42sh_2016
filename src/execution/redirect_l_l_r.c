...
** Last update Sun May 21 23:21:15 2017 Senpai
*/

#include "execution.h"

int	reset_io_l_l_r(int fd_r, int pipefd[2], int newfd_r, int newfd_l_l)
{
  if ((close(fd_r)) == -1)
    return (EXEC_ERROR);
  if ((close(pipefd[0])) == -1)
...
int	change_io_l_l_r(int fd_r, int pipefd[2], char **args, char **envp)
{
  int	signal;
...
int    concatenation_l_l_r(char *redir_l_l, int pipefd[2])
{
  char  *line;
  char  *result;
...
int     redirect_l_l_r(char **args, t_command **command, char **envp)
{
  char  *redir_r;
  char	*redir_l_l;
  int   pipefd[2];
  ...
  if ((pipe(pipefd)) == -1)
    return (EXEC_ERROR);
  if (((concatenation_l_l_r(redir_l_l, pipefd)) == EXEC_ERROR) ||
      ((signal = change_io_l_l_r(fd_r, pipefd, args, envp)) == EXEC_ERROR) ||
      (reset_io_l_l_r(fd_r, pipefd, newfd_r, newfd_l_l)) == EXEC_ERROR)
    return (EXEC_ERROR);
